...
启动后会自动打开浏览器。v1.2 改造版固定使用本地端口 `8766`（原 v1.1 保留 `8765`）；服务只监听 `127.0.0.1`，不会暴露到局域网。
...
- **保密边界**：隐藏目录不是加密。macOS/Linux 会将项目目录收紧为当前账户可访问；备份和项目仍为明文，应仅存放于受控设备，并启用 FileVault、BitLocker 或单位认可的磁盘保护措施。
...
- **v1.2（2026-08）**：前端 V3 重构（三栏工作区、交流修订模式、版本时间线）、项目级自动备份与恢复点、回收站、批次合并暂存提交、单写租约强制接管、固定端口 8766。
- **v1.1（2026-08）**：附件管理增强——共享/取消共享可逆（被引用时确认提示）、文件夹上传去重（同目录同内容复用已有实体）、split-button 关联按钮修复；最近列表后端化（按使用人隔离）；问题清单视图（原项目汇总升级：单位/版块/状态筛选、金额合计、涉及单位数、点击跳转底稿）；全局搜索（单位/底稿/附件模糊匹配，顶栏入口）。原版固定端口为 8765。
- **v1.0（正式版）**：审迹首个正式版。覆盖底稿编写、附件管理、复核流转、归档打包完整闭环，含项目目录伪装、问题分类、版本预览回溯、Excel 导入导出、健康检查、合并导入。
