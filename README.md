...
- **v1.1（2026-08）**：附件管理增强——共享/取消共享可逆（被引用时确认提示）、文件夹上传去重（同目录同内容复用已有实体）、split-button 关联按钮修复；最近列表后端化（按使用人隔离）；问题清单视图（原项目汇总升级：单位/版块/状态筛选、金额合计、涉及单位数、点击跳转底稿）；全局搜索（单位/底稿/附件模糊匹配，顶栏入口）；固定默认端口 8765。
