...
| 🩺 健康检查 | 附件完整性扫描（孤儿/缺失/哈希核对，异步进度可取消）、导入报告、隐私最小化诊断支持包 |
| 🧾 复核与留痕 | 独立草稿恢复、内部复核意见追加事件、按人/动作/日期筛选的永久操作日志导出 |
...
- **删除确认**：任何删除操作前需确认
- **强制使用人**：所有变更类操作写入 `audit_log`，日志随项目走；日志支持按筛选条件导出 CSV
- **支持包最小化**：诊断支持包只含 schema、记录数量和日志链完整性结果；不含项目名称、人员、正文、附件名称、路径或内容
...
- **v1.3（发布候选开发中）**：独立草稿恢复、单位/同单位底稿排序、审迹中台、跨单位复制与模板、批量元数据维护、操作日志筛选导出、隐私最小化诊断支持包；富文本、项目级资料请求、内部复核意见与确认单 DOCX 移入后续版本。
